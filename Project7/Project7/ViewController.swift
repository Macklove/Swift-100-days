...
class ViewController: UITableViewController {
    ...
    @objc func RightBarButtonForText(){
        ...
        let ac = UIAlertController(title: "Введите текст для поиска", message: nil, preferredStyle: .alert)
        ac.addTextField()
        ac.addAction(UIAlertAction(title: "Назад", style: .default, handler: GetBackAllPetitions))
    ...
    public func ShowError() {
        let ac = UIAlertController(title: "Ошибка подключения", message: "Произошла ошибка при установлении соединения. Пожалуйста, проверьте подключение и повторите попытку.", preferredStyle: .alert)
